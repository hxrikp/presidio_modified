...
def recognizers_map():
    rec_map = {
        ...
        "IN_AADHAR": InAadharRecognizer(),
        "IN_VOTER_ID": InVoterIdRecognizer(),
        "IN_UPI": InUpiRecognizer()
    }
    return rec_map
...
def dataset(recognizers_map):
    ...
    # Currently we have 39 sentences, this is a sanity check
    if not len(test_items) == 39:
        raise ValueError(f"expected 39 context sentences but found {len(test_items)}")
